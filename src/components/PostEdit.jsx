...
const PostEdit = ({ onCancel, initialValues }) => {
  const [destino, setDestino] = useState(initialValues.destino || "");
  const [detalhes, setDetalhes] = useState(initialValues.detalhes || "");
  ...
  useEffect(() => {
    ...
    setDetalhes(initialValues.detalhes || "");
  ...
  const handleSubmit = async (event) => {
    ...
    if (fotos.length > 0) {
      fotos.forEach((foto, i) => {
        console.log(`Nova Foto ${i + 1}:`, foto);
    ...
    try {
      ...
      toast.success(response.data.message || "Viagem atualizada com sucesso!");
    } catch (error) {
      toast.error("Ocorreu um erro ao atualizar sua viagem.");
      console.error(error);
    }
  };
  ...
  return (
    <Modal show={openModal} size={"lg"} onClose={onCancel}>
      <Modal.Header>Atualizar Viagem</Modal.Header>
      <Modal.Body>
